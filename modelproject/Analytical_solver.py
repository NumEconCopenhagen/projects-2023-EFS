...
def log_analytic(beta_val=1.0/1.4):
    ...
    # b. define objective function (household utility optimization)
    ...
    # i. characterizes the solution with beta value
...
def steady_state(alpha_val = 0.5,beta_val = 1.0/1.4,n_val = 0.1):
    ...
    # a. fine steady-state capital formula
    steady = ((1-alpha)/((1+n)*(1+1/beta)))**(1/(1-alpha))
    display(steady)

    # b. characterizes the solution with value of beta, alpha and n
    sol_steady = sm.lambdify(args=(alpha,beta,n),expr=steady) # trasforms simpy to function
    opt_k = sol_steady(alpha_val,beta_val,n_val) #solves function for values of beta, n and aplha
    print(f'For alpha = {alpha_val};\nbeta = {beta_val};\nn = {n_val};\nSteady state value of capital, k_ss =')
    display(opt_k)
