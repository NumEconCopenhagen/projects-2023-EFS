...
class OLGModelClass():
    ...
    def setup(self):
        ...
        par.bal_budget = True # imposes a balanced budget (expenses = income)
        ...
        # g. steady-state level for LOG utility and CD production
    ...
    def simulate(self,do_print=True):
        ...
        sim.k_lag[0] = par.K_lag_ini/par.L_lag_ini # capital per-pworker: aggregate capital/workers
...
def simulate_before_s(par,sim,t):
    ...
    if t > 0: # for every period different from the starting point, we update lag variables.
        ...
        sim.L_lag[t] = sim.L_lag[0]*(1+par.n)**t # population grows each period
    ...
    if par.production_function == 'ces':
        ...
        # ii. factor prices, given by profit maximisation FOC's
    ...
    sim.rb[t] = sim.r[t] # the return on bonds is the same
    sim.rt[t] = (1-par.tau_r)*sim.r[t] # after-tax return

    # c. consumption of old in period t
    sim.C2[t] = (1+sim.rt[t])*(sim.K_lag[t]+sim.B_lag[t]) 

    # d. government activities
    sim.T[t] = par.tau_r*sim.r[t]*(sim.K_lag[t]+sim.B_lag[t]) + par.tau_w*sim.w[t]*sim.L_lag[t] #  governement income
    ...
    sim.B[t] = (1+sim.r[t])*sim.B_lag[t] - sim.T[t] + sim.G[t] # new debt required
